...
do_save <- TRUE  # Assuming you have defined do_save somewhere
...
for (parameter in c("pse", "slope")) {
  ...
  if (do_save == TRUE) {
    ...
    ggsave(filename, plot, height = 70, width = 85, units = "mm")
  }
}
...
snew <- coda.samples(
  model =modello1,
  variable.names = c("alpha[1,1]","beta[1,1]"),
  thin = 1,
  n.iter = 5000 )
plot(snew)
...
#quantile
par(mfrow = c(3, 2))
cumuplot(snew, ask = FALSE, auto.layout = FALSE)
